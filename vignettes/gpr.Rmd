---
title: "Script for gpr"
author: "Bayesian Regression with INLA author"
date: "`r Sys.Date()`"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{gpr script}
...
```
```{r}
spde <- inla.spde2.pcmatern(mesh,alpha=alpha,prior.range=c(5,0.1),prior.sigma=c(2,0.1))
formula <- y ~ -1 + f(sinc, model=spde)
resultpc <- inla(formula, data=data,  family="normal",control.predictor= list(A=inla.stack.A(st.est), compute=TRUE))
```
```{r}
library(brinla)
...
formula <- y ~ -1 + f(sinc, model=spde)
resulta <- inla(formula, data=data,  family="normal",control.predictor= list(A=inla.stack.A(st.est),compute=TRUE))
```
```{r}
...
spde <- inla.spde2.matern(mesh, alpha=alpha, constr = FALSE,
                           ...
                           B.kappa = cbind(log(kappa0)))
formula <- y ~ -1 + f(sinc, model=spde)
resultb <- inla(formula, data=data,  family="normal",control.predictor= list(A=inla.stack.A(st.est),compute=TRUE))
```
```{r}
ii <- inla.stack.index(st.est, "est")$data
...
```
```{r}
result <- inla(formula, data=inla.stack.data(sestpred), family="normal",
...
resultl <- inla(formula, data=inla.stack.data(sestpred), family="normal",
 control.predictor= list(A=inla.stack.A(sestpred), compute=TRUE),
 control.family(hyper=list(prec = list(fixed = TRUE, initial = 1e8))))
```
```{r}
...
resulth <- inla(formula, data=inla.stack.data(sestpred), family="normal",
 control.predictor= list(A=inla.stack.A(sestpred), compute=TRUE),
 control.family(hyper=list(prec = list(fixed = TRUE, initial = 1e8))))
```
```{r}
